...
public class ConcertPane extends GridPane {
    ...
    public void submit() {
        ...
        try { event = new Concert04(name, place, dateTime, audience, performers, type, weather);} catch (ParseException ex) {}

        this.getRbWeekEndYes().setSelected( event.isWeekEnd());
        this.getRbWeekEndNo ().setSelected(!event.isWeekEnd());

        this.getRbGoodWeatherYes().setSelected( ((Concert04)event).isGoodWeather());
        this.getRbGoodWeatherNo ().setSelected(!((Concert04)event).isGoodWeather());
